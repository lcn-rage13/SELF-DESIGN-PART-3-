...
shootingstar5Group = createGroup();
  player2.velocityY = 5;
...
if (player2healthbar.width<1){
  textSize(350)
  text("YOU WIN",10,height/2)
  player2.velocity = 0
  shootingstarGroup.destroyEach()
  shootingstar5Group.destroyEach()
}
if (player1healthbar.width<1){
  textSize(300)
  ...
  if (shootingstar5Group.isTouching(player1) && player1healthbar.width > 0.5) {
    player1healthbar.width = player1healthbar.width - 0.5;
  ...
  if (frameCount % 60 == 0) {
    ...
    shootingstar5Group.add(shootingstar5)
    player2sound.play()
  }
  drawSprites();
}